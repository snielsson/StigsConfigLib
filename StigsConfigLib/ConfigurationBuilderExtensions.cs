...
namespace StigsConfigLib {
	public static class ConfigurationBuilderExtensions {
		private static bool TryAddFile(this IConfigurationBuilder @this, string path, bool optional = false,
		...
		public static T Load<T>(this IConfigurationBuilder @this, string env = null, string[] args = null,
			string name = null, string directory = "ConfigurationFiles", bool optional = true,
			bool reloadonChange = true) where T : IConfigurationService<T> {
			...
			if (env != null) @this.Add($"{directory}/{name}.{env}.", optional, reloadonChange);
			@this.AddEnvironmentVariables().AddCommandLine(args);
			var configurationRoot = @this.Build();
			var result = configurationRoot.Get<T>();
			if (reloadonChange) {
				foreach (var provider in configurationRoot.Providers.OfType<FileConfigurationProvider>()
					.Where(x => x.Source.ReloadOnChange)) {
					var path = Path.Combine((provider.Source.FileProvider as PhysicalFileProvider)?.Root ?? "",
						provider.Source.Path);
					provider.GetReloadToken()
						.RegisterChangeCallback(x => result.OnConfigurationChangedEvent(path), path);
				}
			}
